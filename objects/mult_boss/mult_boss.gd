...
@onready var attack_timer = $NextAttack
...
func _ready():
	...
	_set_healthbar(0)
	if !raft:
		raft = get_node_or_null("../player_raft")
		print("raft get failed")
	if raft:
		stun_timer.start(stun_length * 60)

func _perform_attack():
	if not raft:
		print("nope")
		raft = get_parent().get_node_or_null("player_raft")
		return
...
func _on_next_attack_timeout():
	if not is_stunned:
		_perform_attack()
	attack_timer.start(500) #readd randomization
